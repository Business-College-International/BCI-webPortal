...
export interface AssignedAssessment { id: string; title: string; type: string; maxScore: string; weight: string | null; createdAt: string; results: Array<{ id: string; studentId: string; score: string; remark: string | null; enteredAt: string; enteredBy: string }>; }
...
export async function prepareReportCardPublication(studentId: string, termId: string): Promise<ReportCardPublication> { const response = await api.post<ReportCardPublication>('/academic-reports/students/' + encodeURIComponent(studentId) + '/terms/' + encodeURIComponent(termId) + '/publications'); return response.data; }
export async function publishReportCardPublication(id: string): Promise<ReportCardPublication> { const response = await api.post<ReportCardPublication>('/academic-reports/publications/' + encodeURIComponent(id) + '/publish'); return response.data; }
export async function voidReportCardPublication(id: string, reason: string): Promise<ReportCardPublication> { const response = await api.post<ReportCardPublication>('/academic-reports/publications/' + encodeURIComponent(id) + '/void', { reason }); return response.data; }
export async function getCurrentReportCardPublication(studentId: string, termId: string): Promise<ReportCardPublication> { const response = await api.get<ReportCardPublication>('/academic-reports/students/' + encodeURIComponent(studentId) + '/terms/' + encodeURIComponent(termId) + '/publications/current'); return response.data; }
export async function listReportCardPublicationHistory(studentId: string, termId: string): Promise<ReportCardPublication[]> { const response = await api.get<ReportCardPublication[]>('/academic-reports/students/' + encodeURIComponent(studentId) + '/terms/' + encodeURIComponent(termId) + '/publications/history'); return response.data; }
export interface ReportCardCorrectionRequest { id: string; studentId: string; termId: string; targetPublicationId: string; replacementSnapshotJson: Record<string, unknown>; replacementSnapshotHash: string; gradingPolicyVersionId: string | null; reason: string; requestedBy: string; requestedAt: string; decision: 'PENDING' | 'APPROVED' | 'REJECTED'; decidedBy: string | null; decidedAt: string | null; decisionNote: string | null; approvedPublicationId: string | null; createdAt: string; }
export async function requestReportCardCorrection(studentId: string, termId: string, reason: string): Promise<ReportCardCorrectionRequest> { const response = await api.post<ReportCardCorrectionRequest>('/academic-reports/students/' + encodeURIComponent(studentId) + '/terms/' + encodeURIComponent(termId) + '/corrections', { reason }); return response.data; }
export async function listReportCardCorrections(studentId: string, termId: string): Promise<ReportCardCorrectionRequest[]> { const response = await api.get<ReportCardCorrectionRequest[]>('/academic-reports/students/' + encodeURIComponent(studentId) + '/terms/' + encodeURIComponent(termId) + '/corrections'); return response.data; }
export async function approveReportCardCorrection(id: string, decisionNote: string): Promise<{ request: ReportCardCorrectionRequest; voidedPublication: ReportCardPublication; replacementPublication: ReportCardPublication }> { const response = await api.post<{ request: ReportCardCorrectionRequest; voidedPublication: ReportCardPublication; replacementPublication: ReportCardPublication }>('/academic-reports/corrections/' + encodeURIComponent(id) + '/approve', { decisionNote }); return response.data; }
export async function rejectReportCardCorrection(id: string, decisionNote: string): Promise<ReportCardCorrectionRequest> { const response = await api.post<ReportCardCorrectionRequest>('/academic-reports/corrections/' + encodeURIComponent(id) + '/reject', { decisionNote }); return response.data; }
...
export interface ReportCardPublication { id: string; studentId: string; termId: string; publicationVersion: number; status: 'DRAFT' | 'READY_FOR_PUBLICATION' | 'PUBLISHED' | 'VOIDED'; snapshotJson: Record<string, unknown>; snapshotHash: string; gradingPolicyVersionId: string | null; publishedAt: string | null; publishedBy: string | null; voidedAt: string | null; voidedBy: string | null; voidReason: string | null; createdAt: string; }
...
export async function listAssignedAssessments(classId: string, termId: string, subjectId: string): Promise<AssignedAssessment[]> { const response = await api.get<AssignedAssessment[]>('/assessments/assigned', { params: { classId, termId, subjectId } }); return response.data; }
